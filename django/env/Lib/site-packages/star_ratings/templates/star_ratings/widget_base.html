...
<div id="{{ id }}" class="star-ratings" data-max-rating="{{ star_count }}" data-avg-rating="{{ rating.average }}">
    ...
    <div class="star-ratings-rating-average">{% trans 'امتیاز: ' %} <span class='star-ratings-rating-value'>{{ rating.average|floatformat:"-2" }}</span></div>
    ...
    {% block rating_user %}
    <!--<div class="star-ratings-rating-user">{% trans 'امتیازی که شما دادید: ' %} <span class='star-ratings-rating-value' data-when-null="{% trans 'Not rated' %}">{% if user_rating %}{{ user_rating.score }}{% else %}{% trans 'Not rated' %}{% endif %}</span></div>-->
    {% if not user.is_authenticated and not anonymous_ratings and not read_only %}
    <a href="{% url 'account:login' %}">وارد شوید</a> <p> تا بتوانید رای بدهید</p>
    {% endif %}
